...
// Define the order options type to allow size as string or undefined
type OrderOptions = {
  size?: string; // optional string size or undefined
  quantity: number;
};
...
export default function OrderPageClient() {
  ...
  const [orderOptions, setOrderOptions] = useState<OrderOptions>({ size: undefined, quantity: 1 });
    ...
    <div className="min-h-screen bg-zinc-50 flex flex-col md:flex-row items-stretch">
      ...
      <div className="flex-1 flex items-center justify-center bg-white p-8 relative">
        ...
        >
          <Image
            ...
            alt={product.name}
            fill
            className={`object-contain rounded-2xl border shadow transition-transform duration-300 ${
              zoom ? "scale-110 z-20" : "scale-100"
            }`}
      ...
      <div className="flex-1 flex flex-col justify-center px-8 py-12 max-w-xl mx-auto">
        ...
        <div className="text-xs text-zinc-500 mt-2">
          By placing your order you agree to the terms of service. Delivery same day or next day, subject to availability.
        </div>
      </div>
    </div>
  );
